...
def read_session(sess_id: str, db: SessionLocal = Depends(get_db)):
    session = db.query(models.Session).filter_by(id=sess_id).one()
    return session


@app.get("/session/{sess_id}/results")
def get_result(sess_id: str, db: SessionLocal = Depends(get_db)):
    results = db.query(models.Result).filter_by(session_id=sess_id).all()
    return results


@app.get("/result/{res_id}")
def get_results(res_id: str, db: SessionLocal = Depends(get_db)):
