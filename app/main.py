...
@app.get("/")
def read_root(db: SessionLocal = Depends(get_db)) -> Dict:
    return {"Hello": "World"}


@app.get("/sessions")
def read_sessions(db: SessionLocal = Depends(get_db)) -> List:
...
def get_result(sess_id: str, db: SessionLocal = Depends(get_db)) -> List:
